...
class bracDialog : public PIDialog {
private:
...
public:
	bracDialog(vector<ResourceInfo *> rInfos) : PIDialog(),
					   resourceList(), 
					   resourceType(),
					   resourceID(),
					   resourceName(),
...
void bracDialog::Notify(int32 item)
{
...
void bracDialog::GetResourceNames(vector<ResourceInfo *> & resourceInfos)
{
	size_t count = resourceInfos.size();
	if (count == 0) return;
