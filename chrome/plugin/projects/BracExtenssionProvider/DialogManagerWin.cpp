...
void DialogManagerWin::OpenFileDialog(const FB::BrowserHostPtr& host, FB::PluginWindow* win, const std::string& path, const std::string& filter, const PathCallback& cb) {
	FB::PluginWindowWin* wndWin = dynamic_cast<FB::PluginWindowWin*>(win);
	FB::PluginWindowlessWin* wndlessWin = dynamic_cast<FB::PluginWindowlessWin*>(win);

	HWND browserWindow = wndWin ? wndWin->getBrowserHWND() : wndlessWin->getHWND();
	boost::thread dlgThread(&DialogManagerWin::_showFileDialog, this, browserWindow, path, filter, cb);
}
...
void DialogManagerWin::_showFileDialog(HWND wnd, const std::string& path, const std::string& filter, const PathCallback& cb)
{
	...
	CStringW filters = wFilter.c_str();
	filters.Replace('|', '\0');

    OPENFILENAME opf;
    opf.hwndOwner = wnd;
    opf.lpstrFilter = filters;
